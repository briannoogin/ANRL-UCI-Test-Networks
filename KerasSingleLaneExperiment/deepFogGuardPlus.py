...
def define_adjusted_deepFogGuardPlus(num_vars,num_classes,hidden_units,survive_rates,skip_hyperconnections = [1,1,1]):
    """Define a deepFogGuardPlus model.
    ### Naming Convention
        ex: f2f1 = connection between fog node 2 and fog node 1
    ### Arguments
    ...
    ef2_dropout_multiply = Lambda(lambda x : K.switch(K.learning_phase(), x, x * e_survive_rate),name = 'ef2_dropout_lambda')
    ef1_dropout_multiply = Lambda(lambda x : K.switch(K.learning_phase(), x, x * e_survive_rate),name = 'ef1_dropout_lambda')
    f2f1_dropout_multiply= Lambda(lambda x : K.switch(K.learning_phase(), x, x * f2_survive_rate),name = 'f2f1_dropout_lambda')
    f2c_dropout_multiply= Lambda(lambda x : K.switch(K.learning_phase(), x, x * f2_survive_rate),name = 'f2c_dropout_lambda')
    f1c_dropout_multiply = Lambda(lambda x : K.switch(K.learning_phase(),x, x * f1_survive_rate),name = 'f1c_dropout_lambda')
    # one input layer
    IoT_node = Input(shape = (num_vars,))
    ...
    e = e_failure_lambda(e)
    ef2 = multiply_weight_layer_ef2(e)
    # multiply based on dropout p
    ef2 = ef2_dropout_multiply(e)
    # use a linear Dense layer to transform input into the shape needed for the network
    duplicated_input = Dense(units=hidden_units,name="duplicated_input",activation='linear')(IoT_node)
    IoTf2 = multiply_weight_layer_IoTf2(duplicated_input)
    connection_f2 = Lambda(add_node_layers,name="F2_Input")([ef2,IoTf2])
    ...
    ef1 = multiply_weight_layer_ef1(e)
    # multiply based on dropout p
    ef1 = ef1_dropout_multiply(ef1)
    f2f1 = multiply_weight_layer_f2f1(f2)
    # multiply based on dropout p
    f2f1 = f2f1_dropout_multiply(f2f1)
    connection_f1 = Lambda(add_node_layers,name="F1_Input")([ef1,f2f1])
    ...
    f2c = multiply_weight_layer_f2c(f2)
    # multiply based on dropout p
    f2c = f2c_dropout_multiply(f2c)
    f1c = multiply_weight_layer_f1c(f1)
    # multiply based on dropout p
    f1c = f1c_dropout_multiply(f1c)
    connection_cloud = Lambda(add_node_layers,name="Cloud_Input")([f2c,f1c])
